...
export class RecipeScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.add.text(width / 2, 80, 'ALCHEMY ENCYCLOPEDIA', {
            fontSize: '42px',
            color: '#00ffff',
            fontStyle: 'bold'
        }).setOrigin(0.5);
        ...
        RECIPES.forEach((r, i) => {
            const y = startY + (i * 50);
            this.add.text(col1X, y, r.name, { fontSize: '24px', color: '#ffd700' });
            this.add.text(col2X, y, r.combo, { fontSize: '24px', color: '#ffffff' });
        });

        const closeBtn = this.add.text(width / 2, height - 80, '[ PRESS E OR CLICK TO CLOSE ]', {
            fontSize: '28px',
            color: '#aaaaaa'
