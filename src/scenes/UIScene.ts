...
export class UIScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.hpBar = this.add
            .rectangle(hpX + 4, 0, fullWidth - 8, barHeight - 8, 0xffcc00)
            .setOrigin(0, 0);
        this.hpShine = this.add.graphics();
        this.hpIcon = this.add
            .image(hpX - 1, 0, 'hp_icon')
        ...
        this.mpBar = this.add
            .rectangle(hpX + 4, 0, fullWidth - 8, barHeight - 8, 0x0099ff)
            .setOrigin(0, 0);
        const mpShineGraphic = this.add.graphics();
        const mpIconImg = this.add
            .image(hpX - 1, 0, 'dungeon', 'flask_big_blue')
            .setDisplaySize(40, 40)
            .setOrigin(0.5, 0.5)
            .setDepth(20);
        ...
        this.expIcon = this.add
            .image(hpX - 1, 0, 'dungeon', 'flask_big_green')
            .setDisplaySize(40, 40)
            .setOrigin(0.5, 0.5)
            .setDepth(20);
        ...
        this.bossHpBar = this.add
            .rectangle(-bossBarW / 2 + 3, 0, bossBarW - 6, bossBarH - 6, 0xff0000)
            .setOrigin(0, 0.5);
        ...
        const mmBg2 = this.add
            .rectangle(0, 0, this.MINIMAP_SIZE, this.MINIMAP_SIZE, 0x333333, 0.9)
            .setOrigin(1, 0);
        this.minimapGraphics = this.add.graphics();
        this.arrowGraphics = this.add.graphics();
        const mmHitArea = this.add
            .rectangle(0, 0, this.MINIMAP_SIZE, this.MINIMAP_SIZE, 0x000000, 0)
            .setOrigin(1, 0)
        ...
        const pauseText = this.add
            ...
            .setVisible(false)
            .setDepth(901);
        const resumeBtnBg = this.add
            .rectangle(640, 400, 300, 60, 0x3d2b1f, 0.9)
        ...
        const resumeBtnText = this.add
            ...
            .setVisible(false)
            .setDepth(902);
        const quitBtnBg = this.add
            .rectangle(640, 480, 300, 60, 0x3d2b1f, 0.9)
        ...
        const setPauseVisible = (v: boolean) => {
            ...
            pauseText.setVisible(v);
            resumeBtnBg.setVisible(v);
            resumeBtnText.setVisible(v);
        ...
        resumeBtnBg.on('pointerdown', () => {
            this.scene.get('MainScene').scene.resume();
            setPauseVisible(false);
        });
        ...
        this.events.once(Phaser.Scenes.Events.SHUTDOWN, () => {
            ...
            window.removeEventListener(
                'xp_percent_collected',
                this.handleXpPercent as EventListener,
            );
    ...
    private toggleMinimap() {
        ...
        if (this.isMinimapEnlarged) {
            ...
            this.tweens.add({
                ...
                x: 640 + (this.MINIMAP_SIZE * 3.5) / 2,
                y: 360 - (this.MINIMAP_SIZE * 3.5) / 2,
                scale: 3.5,
                duration: 500,
                ease: 'Cubic.easeOut',
            });
    ...
    private updateTargetArrows() {
        ...
        for (let i = 0; i < bosses.length; i++)
            targets.push({ x: Position.x[bosses[i]], y: Position.y[bosses[i]], color: 0xff00ff });
        ...
        if (this.spawningComplete && bosses.length === 0) {
            ...
            if (enemies.length > 0 && enemies.length < 5) {
                for (let i = 0; i < enemies.length; i++)
                    targets.push({
                        x: Position.x[enemies[i]],
                        y: Position.y[enemies[i]],
                        color: 0xff0000,
                    });
            }
        }
        ...
        for (const t of targets) {
            ...
            this.arrowGraphics.fillTriangle(
                ax + Math.cos(angle) * 8,
                ay + Math.sin(angle) * 8,
                ax + Math.cos(angle + 2.4) * 5,
                ay + Math.sin(angle + 2.4) * 5,
                ax + Math.cos(angle - 2.4) * 5,
                ay + Math.sin(angle - 2.4) * 5,
            );
        }
        ...
        targets.forEach((t) => {
            ...
            if (Math.sqrt(dx * dx + dy * dy) > 300) {
                ...
                const pX = 640 + Math.cos(angle) * 280;
                const pY = 360 + Math.sin(angle) * 280;
                const bLX = 640 + Math.cos(angle - 0.2) * (280 - 24);
                const bLY = 360 + Math.sin(angle - 0.2) * (280 - 24);
                const bRX = 640 + Math.cos(angle + 0.2) * (280 - 24);
                const bRY = 360 + Math.sin(angle + 0.2) * (280 - 24);
    ...
    private updateMinimap() {
        ...
        const players = this.playerQuery(world);
        if (players.length > 0) {
            const tX = Math.floor(Position.x[players[0]] / 16);
            const tY = Math.floor(Position.y[players[0]] / 16);
            for (let dy = -15; dy <= 15; dy++) {
                for (let dx = -15; dx <= 15; dx++) {
                    if (dx * dx + dy * dy <= 225) {
                        ...
                        if (
                            ny >= 0 &&
                            ny < this.dungeonMap.length &&
                            nx >= 0 &&
                            nx < this.dungeonMap[0].length
                        )
        ...
        const scale =
            this.MINIMAP_SIZE /
            Math.max(this.dungeonMap[0].length * 16, this.dungeonMap.length * 16);
        const oX = -2 - this.MINIMAP_SIZE;
        const oY = 2;

        for (let y = 0; y < this.dungeonMap.length; y++) {
            for (let x = 0; x < this.dungeonMap[0].length; x++) {
                if (this.dungeonMap[y][x] === 1)
                    this.minimapGraphics.fillStyle(
                        this.discoveredMap[y][x] ? 0x888888 : 0x222222,
                        1.0,
                    );
        ...
        for (let i = 0; i < enemies.length; i++) {
            const eid = enemies[i];
            const ex = oX + Position.x[eid] * scale;
            const ey = oY + Position.y[eid] * scale;
            if (hasComponent(world, Boss, eid)) {
                this.minimapGraphics.fillStyle(0xff00ff, 1).fillCircle(ex, ey, 4);
            } else {
                this.minimapGraphics.fillStyle(0xff0000, 0.8).fillRect(ex - 1, ey - 1, 2, 2);
            }
        }
        if (players.length > 0) {
            ...
            this.minimapGraphics
                ...
                .fillCircle(oX + Position.x[peid] * scale, oY + Position.y[peid] * scale, 3);
        }
    }
                ...
                if (hasComponent(world, Mana, peid)) {
                    ...
                    window.dispatchEvent(
                        new CustomEvent('mp_updated', {
                            detail: { current: Mana.current[peid], max: Mana.max[peid] },
                        }),
                    );
                }
    ...
    private handleHp = (e: CustomEvent<{ current: number; max: number }>) => {
        ...
        this.hpBar.setFillStyle(p > 0.2 ? 0xffcc00 : 0xff0000);
    };
    ...
    private handleBossHp = (e: CustomEvent<{ current: number; max: number; name?: string }>) => {
        ...
        this.bossHpBar.displayWidth =
            (600 - 6) * Phaser.Math.Clamp(e.detail.current / e.detail.max, 0, 1);
    ...
    private syncInitialState() {
        ...
        if (mainScene) {
            ...
            if (mainScene.dungeon && mainScene.dungeon.map && mainScene.dungeon.map.length > 0) {
                ...
                if (
                    this.discoveredMap.length !== h ||
                    (h > 0 && this.discoveredMap[0].length !== w)
                ) {
                    this.discoveredMap = Array.from({ length: h }, () => Array(w).fill(false));
                }
            }
        ...
        if (players.length > 0) {
            ...
            if (hasComponent(world, Mana, pid)) {
                this.handleMp({
                    detail: { current: Mana.current[pid], max: Mana.max[pid] },
                } as any);
            }
        }
        this.updateStageLevelText();
    ...
    private repositionUIBars() {
        ...
        this.expFrame
            ...
            .strokeRoundedRect(hpX, expY - barHeight, fullWidth, barHeight, 4)
            .fillStyle(0x000000, 0.8)
            .fillRoundedRect(hpX, expY - barHeight, fullWidth, barHeight, 4);
        this.xpBar.setPosition(hpX + 4, expY - barHeight + 4);
        this.xpBar.height = barHeight - 8;
        this.expShine
            .clear()
            .fillStyle(0xffffff, 0.2)
            .fillRect(hpX + 4, expY - barHeight + 4, fullWidth - 8, (barHeight - 8) / 2);
        ...
        if (needsMana) {
            ...
            mpFrame
                .clear()
                .lineStyle(4, 0xffffff)
                .strokeRoundedRect(hpX, -barHeight, fullWidth, barHeight, 4)
                .fillStyle(0x000000, 0.8)
                .fillRoundedRect(hpX, -barHeight, fullWidth, barHeight, 4);
            this.mpBar.setPosition(hpX + 4, -barHeight + 4);
            this.mpBar.height = barHeight - 8;
            mpShine
                .clear()
                .fillStyle(0xffffff, 0.2)
                .fillRect(hpX + 4, -barHeight + 4, fullWidth - 8, (barHeight - 8) / 2);
        ...
        this.hpFrame
            .clear()
            .lineStyle(4, 0xffffff)
            .strokeRoundedRect(hpX, hpY - barHeight, fullWidth, barHeight, 4)
            .fillStyle(0x000000, 0.8)
            .fillRoundedRect(hpX, hpY - barHeight, fullWidth, barHeight, 4);
        this.hpBar.setPosition(hpX + 4, hpY - barHeight + 4);
        this.hpBar.height = barHeight - 8;
        this.hpShine
            .clear()
            .fillStyle(0xffffff, 0.2)
            .fillRect(hpX + 4, hpY - barHeight + 4, fullWidth - 8, (barHeight - 8) / 2);
