...
### Phase 14: 추가 기믹 및 밸런싱 (Completed)

- [x] **보스 공격 개편**: 보스의 범위 공격이 화면 끝까지 가지 않고 한정된 반경 내에서만 동작하도록 수정.
- [x] **함정 데미지 편의성 개선**: 가시 함정이 플레이어뿐만 아니라 몬스터에게도 데미지/에너지 소모를 유발하도록 시스템 확장.
- [x] **충돌/스폰 개선**: 적이 1x1 타일에 끼이거나 벽에 박혀서 스폰되지 않도록 3x3 클리어런스 체크(`hasClearance`) 추가.
- [x] **Juice 최적화**: 화면 흔들림(Screen Shake) 강도를 전반적으로 50% 수준으로 대폭 감소.
- [x] **BGM 고도화**: 중세풍 테마 적용 (`select_bgm` 추가 로드, `main_bgm`, `boss_bgm` 자동 전환 시스템 구축).
- [x] **무기 연출 강화**: 전사(검 휘두르기), 엘프(활 방향 로테이션), 마법사(지팡이 범위 타겟팅) 무기 애니메이션 적용 완료.

### Phase 15: 최종 연출 및 수집 요소 (Next Sprint)
