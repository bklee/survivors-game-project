...
- [x] **공격 자동화 (Auto-Attack)**: 마법 발동 시 매번 Spacebar를 누르는 대신, 쿨타임에 맞춰 근처 적을 자동으로 조준/공격하는 오토 시스테 구현.
- [x] **연금술 조합 로직 제거**: 게임 플레이 편의성과 난이도 조절을 위해 불필요했던 복잡한 연금술 큐 및 조합 시스템을 과감히 소거.
- [x] **스테이지별 몬스터 그룹핑 (Monster Grouping)**: 스테이지별로 고유한 몬스터(Undead -> Orc -> Demon)가 출현하도록 그룹핑 완료.
- [x] **하이퀄리티 GIF 몬스터 스프라이트**: `demons`, `orcs`, `undeads` GIF 기반의 생동감 넘치는 애니메이션 프레임 정밀 로드 및 RenderSystem 연동.
...
- [x] **비주얼 연출 폴리싱 (No Flash)**: 과도한 화면 번쩍거림(White Flash) 효과를 제거하여 오랫동안 플레이해도 눈이 편안한 환경 조성.
