...
- [x] **무기 연출 강화**: 전사(검 휘두르기, slash 이펙트 추가), 엘프(활 방향 로테이션), 마법사(지팡이 범위 타겟팅) 무기 애니메이션 적용 완료.
- [x] **몬스터 웨이브 밸런싱**: 무한 스폰을 방지하기 위해 Wave별 몬스터 최대치(maxEnemies) 제한 추가.
- [x] **초기 안전 구역 확보**: 플레이어 스폰 위치 기준 최소 스폰 거리를 대폭 늘려(500~1000px) 시작 시 안전 지대(Safe Zone) 확보.
- [x] **하이퀄리티 BGM**: 8비트를 넘어선 오케스트라/판타지풍 BGM 연동 (선택창: `hero_reprise`, 전장: `fight_for_better_future`, 보스: `boss_battle_...`).
- [x] **맵 보급품 배치**: 플레이어 체력을 즉시 40 회복시켜주는 `초록/빨강 체력 회복 물약(Health Potion)` 및 먹으면 1000 이상(1500)의 대량의 경험치를 드랍하는 `보물 상자(Treasure Chest)` 추가 적용.
- [x] **미니맵 디자인 개선**: 기존 반투명한 UI에서 검은색 벽, 하얀색 바닥으로 변경하여 레이더망의 시인성을 압도적으로 개선.
- [x] **불필요한 맵 프롭 제거**: 충돌 및 렌더링 버그(마법사 잔상/레버/문)를 유발하던 필요 이상의 동적 맵 기믹(엘리트 가드 시스템 등)들을 대거 소거하여 코어 플레이에 집중.
- [x] **셀룰러 오토마타 맵(Cellular Automata Map) 랜덤 생성**: 맵의 모든 요소를 다양하게 결합해 스테이지가 넘어갈 때 마다 유기적이고 복잡한 동굴/기둥 형태의 맵을 매번 랜덤으로 새롭게 자동 생성.
- [x] **스테이지 클리어 및 3스테이지 보스 구조 개선**: 전체 몹을 모두 처치하면 스테이지가 클리어되며, 3스테이지마다 등장하는 보스를 처치해야만 다음 스테이지로 넘어갈 수 있도록 룰셋 정립.
